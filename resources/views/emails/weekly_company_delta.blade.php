<h2>Sedmični izvještaj (razlike vrijednosti)</h2>
<p>Period: {{ $start->format('d.m.Y H:i') }} - {{ $end->format('d.m.Y H:i') }}</p>
    ...
    <table border="1" cellpadding="6" cellspacing="0" width="100%" style="border-collapse: collapse; font-family: Arial, sans-serif; font-size: 14px;">
        <thead>
            <tr>
                ...
                <th align="right">Prvi unos</th>
                <th align="right">Zadnji unos</th>
        ...
        <tbody>
            @forelse($companyData['instruments'] as $row)
                <tr>
                    ...
                    <td align="right">
                        @if($row['first'])
                            {{ number_format((float)$row['first']->vrijednost, 2, ',', '.') }} ({{ optional($row['first']->datum)->format('d.m.Y') }} {{ optional($row['first']->vrijeme)->format('H:i') }})
                        @else
                            —
                        @endif
                    </td>
                    <td align="right">
                        @if($row['last'])
                            {{ number_format((float)$row['last']->vrijednost, 2, ',', '.') }} ({{ optional($row['last']->datum)->format('d.m.Y') }} {{ optional($row['last']->vrijeme)->format('H:i') }})
                        @else
                            —
                        @endif
                    </td>
